...
```
Sim! Podemos prosseguir com a limpeza do dataframe. Utilizamos a função de inner join para substituir o id do município em SSP pelo seu nome, que está presente apenas no dataset MUN2. Substituimos as colunas de ano e mes por uma com a data completa no formato lubridate, deixando 1 como o dia padrão. Também percebemos que a partir da data 09/2021, todas as entradas são NA, então retiramos esses registros.
...
ssp2 <- ssp %>%
  ...
  select(-ano, -mes) %>%  # remove as colunas de mes e ano 
...
ssp2 %>% 
  ...
  ggplot(aes(x=data))+
  geom_line(aes(y=total_de_roubo_outros))+
  geom_line(aes(y=furto_outros, color="red"))
  
```
```{r}
ssp2 %>% 
  filter(nome == "Campinas") %>%
  ggplot(aes(x=data))+
  geom_line(aes(y=roubo_de_veiculo))+
  geom_line(aes(y=furto_de_veiculo, color="red"))
```
